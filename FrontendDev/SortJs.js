...
const sortViewCountTags = document.querySelectorAll(".dropdown-item");
sortViewCountTags.forEach((element) => {
  element.addEventListener("click", (event) => {
    const sortType = event.target.id;
    if (sortType === "ascending") {
      // sort ascending
      const viewCountSortedByAscendingOrder = videos.sort(
        (a, b) => a.viewCount - b.viewCount);
      updateSortUI(viewCountSortedByAscendingOrder);
    } else {
      // sort descending
      const viewCountSortedByDescendingOrder = videos.sort(
        (a, b) => b.viewCount - a.viewCount);
      updateSortUI(viewCountSortedByDescendingOrder);
    }
  });
});
...
const updateSortUI = (videos) => {
  ...
  `;
    containerTag.innerHTML += videoTag; // Don't forget + before assign (=)
  });
};
